...
let dgWs: WebSocket | null = null; // Deepgram STT WebSocket
    ...
    try {
        ...
        recorder.onAudioReady((event: { buffer: RNAudioBuffer; numFrames: number; when: number }) => {
            ...
            if (activeSttProvider === 'deepgram' && dgWs && dgWs.readyState === WebSocket.OPEN) {
                // Deepgram expects raw PCM16 binary
                const pcm16 = float32ToPcm16Bytes(channelData);
                dgWs.send(pcm16);
            } else {
...
function float32ToPcm16Bytes(float32: Float32Array): ArrayBuffer {
...
function float32ToBase64Pcm16(float32: Float32Array): string {
    const bytes = new Uint8Array(float32ToPcm16Bytes(float32));
    let binary = '';
...
function connectDeepgram(apiKey: string, language?: string): Promise<void> {
    return new Promise((resolve, reject) => {
        ...
        // Pass API key as query param since RN WebSocket doesn't support custom headers
...
function closeDeepgram() {
    if (dgWs) {
        // Send empty byte to flush, then close
...
class RealtimeVoiceSessionImpl implements VoiceSession {
    ...
    async startSession(config: VoiceSessionConfig): Promise<void> {
        ...
        try {
            ...
            storage.getState().setRealtimeStatus('error');
            closeDeepgram();
            // ws may have been set in sub-methods before they threw
    ...
    stopTalking(): void {
        ...
        if (activeSttProvider === 'deepgram') {
            ...
            // Send empty buffer to flush Deepgram
            try { dgWs.send(new ArrayBuffer(0)); } catch {}
        } else {
            if (!ws || ws.readyState !== WebSocket.OPEN) return;
